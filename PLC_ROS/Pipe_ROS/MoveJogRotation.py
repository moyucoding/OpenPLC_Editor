...
from ros_motion.action import MotionMoveJogRotation

class MoveJogRotationActionClient(Node):

    def __init__(self):
        super().__init__('Motion/MoveJogRotation')
        
        self._action_client = ActionClient(self, MotionMoveJogRotation, 'Motion/MoveJogRotation')
        self._ret = 0
        ...
        while True:
            try:
                ...
                if data.decode('utf-8').split(';')[0] == 'MoveJogRotation':
                    ...
                    #Create a goal
                    goal = MotionMoveJogRotation.Goal()
                    goal.id = 1
